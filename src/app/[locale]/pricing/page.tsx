...
const initialPricingPlansData = [
  {
    ...
    prices: {
      USD: { monthly: "$49", annually: "$468" }, // $39/month if annual
  ...
  },
  {
    ...
    prices: {
      USD: { monthly: "$149", annually: "$1428" }, // $119/month if annual
      IDR: { monthly: "Rp 2.250.000", annually: "Rp 21.600.000" }, // Rp 1.800.000/bulan if annual
    },
    frequencies: {
      USD: { monthly: "/month", annually: "/year" },
      IDR: { monthly: "/bulan", annually: "/tahun" },
  ...
  },
  {
    ...
    prices: { // Enterprise price is custom, so it doesn't change with currency/cycle
...
export default function PricingPage() {
  ...
  const pricingPlans = initialPricingPlansData.map(plan => ({
    ...
    annualDiscount: plan.annualDiscountKey ? t(plan.annualDiscountKey as any) : undefined,
  }));
    ...
    <div className="container mx-auto px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-col sm:flex-row justify-center items-center gap-4 mb-10">
        ...
        <Select value={currency} onValueChange={(value: 'USD' | 'IDR') => setCurrency(value)}>
          ...
          <SelectContent className="bg-popover">
            <SelectItem value="USD" className="cursor-pointer">{t('usd')}</SelectItem>
            <SelectItem value="IDR" className="cursor-pointer">{t('idr')}</SelectItem>
          </SelectContent>
        </Select>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {pricingPlans.map(plan => (
          <Card key={plan.id} className={`flex flex-col shadow-lg hover:shadow-2xl transition-shadow duration-300 ${plan.popular ? 'border-2 border-primary ring-2 ring-primary/20' : ''}`}>
            {plan.popular && (
              <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2">
                <span className="bg-primary text-primary-foreground px-3 py-1 text-sm font-semibold rounded-full shadow-md uppercase">{t('popular')}</span>
              </div>
            )}
            <CardHeader className="pt-8">
              ...
              <CardDescription className="text-center min-h-[3em] mt-2">
                {plan.description}
                {billingCycle === 'annually' && plan.annualDiscount && plan.id !== "enterprise" && (
            ...
            <CardFooter>
              <Button asChild className={`w-full ${plan.popular ? 'bg-primary hover:bg-primary/90 text-primary-foreground' : 'bg-accent hover:bg-accent/90 text-accent-foreground'}`} size="lg">
      ...
      <div className="text-center mt-16">
        <h2 className="text-2xl font-semibold font-headline text-foreground">{t('customSolutionTitle')}</h2>
